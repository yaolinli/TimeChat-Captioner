...
import av
def _check_if_video_has_audio(inputs, video_path):
    if isinstance(video_path, list): # 输入为视频帧列表
        if inputs.audio_in_video != []: # audio_in_video不为空，则有音频存在
            return True
        else: # audio_in_video为空，没有音频存在
            print("inputs:",inputs)
            sleep(10)
            assert False #终止掉这个异常
            return False
...
class Qwen2_5OmniTemplate(Qwen2_5VLTemplate):
    ...
    def replace_tag(self, media_type: Literal['image', 'video', 'audio'], index: int,
        ...
        if media_type == 'image':
            ...
            video = inputs.videos[index]
            _video, video_fps = fetch_video({'video': video}, return_video_sample_fps=True) # NOTE 在inputs内添加了video_kwargs

            if isinstance(_video, torch.Tensor):
                _video = _video.to(torch.uint8)
            elif isinstance(inputs.videos[index], list): # 输入为视频帧列表
                pass # TODO
            else:
                raise ValueError()
            inputs.videos[index] = _video
            inputs.video_kwargs.update({'fps': video_fps})
            if self.use_audio_in_video and _check_if_video_has_audio(inputs, video):
                import librosa
                if isinstance(video, list): # 输入为视频帧列表
                    audio_in_video = librosa.load(inputs.audio_in_video[index], sr=self.sampling_rate)[0]
                    inputs.audios.insert(inputs.audio_idx, (audio_in_video, 'video'))
                    inputs.audio_idx += 1
                else: 
                    # 视频有音频流才继续加载音频
            ...
            elif self.use_audio_in_video and not _check_if_video_has_audio(inputs, video):
                assert False,"use_audio_in_video为True,但是没有音频"
                if self.version == 'omni_v2_5':
    ...
    def _encode(self, inputs: StdTemplateInputs) -> Dict[str, Any]:
        ...
        media_inputs = processor(
            ...
            do_resize=do_resize,
            return_tensors='pt',
            **inputs.video_kwargs) # 添加**video_kwargs，确保位置编码与绝对时间一致
