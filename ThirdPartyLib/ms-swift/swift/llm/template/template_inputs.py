...
class StdTemplateInputs:
    ...
    images: List[Union[str, Image.Image]] = field(default_factory=list)
    videos: List[str] = field(default_factory=list)
    audio_in_video: List[str] = field(default_factory=list)
    audios: List[str] = field(default_factory=list)
    video_kwargs: Dict[str, List[Any]] = field(default_factory=dict)
    objects: Dict[str, List[Any]] = field(default_factory=dict)
    ...
    def __post_init__(self):
        ...
        if self.audio_in_video and not isinstance(self.audio_in_video, (list, tuple)):
            self.audio_in_video = [self.audio_in_video]
    ...
    def remove_messages_media(messages: Messages) -> Dict[str, Any]:
        res = {'images': [], 'audios': [], 'videos': [], 'audio_in_video': []}
        for message in messages:
